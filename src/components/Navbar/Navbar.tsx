        ...
        <>
            <header className="w-full bg-custom-base h-14 px-2.5">
                <nav className="w-full h-full flex items-center justify-between">
                    ...
                    {
                        open
                        ? <XIcon className="w-7 h-7 text-black cursor-pointer" onClick={() => setOpen(false)}/>
                        : <MenuIcon className="w-6 h-6 text-black cursor-pointer" onClick={() => setOpen(true)}/>
                    }
                </nav>
            </header>
            <Separator orientation="horizontal"/>
            <div className={`absolute top-0 -translate-y-full w-full h-72 bg-custom-base p-3 flex flex-col items-center justify-between transition-all duration-500 ${open ? "top-14.5 translate-y-0" : ""}`}>
